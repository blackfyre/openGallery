...
class Sitemap {
     ...
     * @param null|array $alternates An array of alternate language versions of the same page $lang=>$link
     * @return Sitemap
     */
    public function addItem($loc, $priority = self::DEFAULT_PRIORITY, $changefreq = NULL, $lastmod = NULL, $alternates = NULL) {
        ...
        if (is_array($alternates)) {
            foreach ($alternates AS $lang=>$link) {
                $this->getWriter()->writeRaw('<xhtml:link
    rel="alternate"
    hreflang="' . $lang . '"
    href="' . $this->getDomain() . $link . '" />
    ');
            }
        }
